...
func InitDBFromPath(path string) error {
	...
	err = DB.AutoMigrate(model.Server{}, model.User{}, model.ServerGroup{}, model.NotificationGroup{},
		...
		model.WAF{}, model.Oauth2Bind{}, model.AutoSSH{}, model.UserServer{},
		model.TerminalSession{}, model.TerminalCommand{}, model.TerminalBlacklist{})
	if err != nil {
		return err
	}

	// 迁移现有的 Server.UserID 关系到 UserServer 表
	...
	if err := DB.Find(&servers).Error; err == nil {
		for _, server := range servers {
			if server.UserID > 0 {
				// 检查关联是否已存在
				var count int64
				DB.Model(&model.UserServer{}).
					Where("user_id = ? AND server_id = ?", server.UserID, server.ID).
					Count(&count)

				if count == 0 {
					DB.Create(&model.UserServer{
