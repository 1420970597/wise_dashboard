...
// checkServerPermission 检查用户是否有权限访问服务器
// 支持多对多关系和向后兼容的 UserID 字段
func checkServerPermission(c *gin.Context, serverID uint64) bool {
	...
	// 检查多对多关联表
	var count int64
	singleton.DB.Model(&model.UserServer{}).
		Where("user_id = ? AND server_id = ?", user.ID, serverID).
		Count(&count)
...
func forceUpdateServer(c *gin.Context) (*model.ServerTaskResponse, error) {
	...
	for _, sid := range forceUpdateServers {
		...
		if server != nil && server.TaskStream != nil {
			if !checkServerPermission(c, sid) {
				return nil, singleton.Localizer.ErrorT("permission denied")
			}
