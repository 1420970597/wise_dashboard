...
func listUser(c *gin.Context) ([]UserWithServers, error) {
	...
	// 获取每个用户的服务器关联
	var result []UserWithServers
	for _, user := range users {
		var userServers []model.UserServer
		singleton.DB.Where("user_id = ?", user.ID).Find(&userServers)

		serverIDs := make([]uint64, 0, len(userServers))
		for _, us := range userServers {
			serverIDs = append(serverIDs, us.ServerID)
		}

		result = append(result, UserWithServers{
			User:      user,
			ServerIDs: serverIDs,
		})
	}
...
func createUser(c *gin.Context) (uint64, error) {
	...
	// 创建用户-服务器关联
	if len(uf.ServerIDs) > 0 {
		for _, serverID := range uf.ServerIDs {
			singleton.DB.Create(&model.UserServer{
				UserID:   u.ID,
				ServerID: serverID,
			})
		}
...
// @Param body body model.UserForm true "UserForm"
// @Produce json
// @Success 200 {object} model.CommonResponse[any]
...
func updateUser(c *gin.Context) (any, error) {
	...
	if err := singleton.DB.First(&user, userID).Error; err != nil {
		return nil, singleton.Localizer.ErrorT("user id %d does not exist", userID)
	}
	...
	user.Role = model.Role(uf.Role)

	// 更新密码（如果提供）
	if uf.Password != "" {
		user.Password = uf.Password
		if err := user.BeforeSave(singleton.DB); err != nil {
			return nil, newGormError("%v", err)
		}
	}
	...
	if uf.ServerIDs != nil {
		// 删除现有关联
		if err := singleton.DB.Where("user_id = ?", userID).Delete(&model.UserServer{}).Error; err != nil {
			return nil, newGormError("%v", err)
		}

		// 创建新关联
		for _, serverID := range uf.ServerIDs {
			if err := singleton.DB.Create(&model.UserServer{
				UserID:   userID,
				ServerID: serverID,
			}).Error; err != nil {
				return nil, newGormError("%v", err)
...
// @Summary Update user server permissions
// @Security BearerAuth
// @Schemes
// @Description Update user server permissions
// @Tags admin required
// @Accept json
// @Param id path uint true "User ID"
// @Param body body []uint64 true "Server IDs"
// @Produce json
// @Success 200 {object} model.CommonResponse[any]
// @Router /user/{id}/servers [put]
func updateUserServers(c *gin.Context) (any, error) {
	idStr := c.Param("id")
	userID, err := strconv.ParseUint(idStr, 10, 64)
	if err != nil {
		return nil, err
	...
	if err := c.ShouldBindJSON(&serverIDs); err != nil {
		return nil, err
	}
